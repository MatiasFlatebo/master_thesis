...
class DDPMTEDiScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def _get_variance(self, t, predicted_variance=None, variance_type=None):
        ...
        # for rl-diffuser https://arxiv.org/abs/2205.09991
            ...
            # Throw not implemented error here
            raise NotImplementedError("learned_range is not implemented for DDPMSTEDicheduler")
    ...
    def step(
        ...
        **kwargs,
        ...
        Args:
            ...
            timestep (B, T), torch.long: current discrete timestep in the diffusion chain.
            sample (`torch.FloatTensor`):
        ...
        # Move relevant tensors to the same device as the model output
        self.alphas_cumprod = self.alphas_cumprod.to(device=model_output.device, dtype=model_output.dtype)
        self.alphas = self.alphas.to(device=model_output.device, dtype=model_output.dtype)
        self.betas = self.betas.to(device=model_output.device, dtype=model_output.dtype)
        ...
        pred_original_sample_coeff = (alpha_prod_t_prev.unsqueeze(-1) ** (0.5) * self.betas[t].unsqueeze(-1)) / beta_prod_t #(B, T, 1)
        current_sample_coeff = self.alphas[t].unsqueeze(-1) ** (0.5) * beta_prod_t_prev.unsqueeze(-1) / beta_prod_t #(B,T,1)
        ...
        if self.variance_type == "fixed_small_log":
            variance = self._get_variance(t, predicted_variance=predicted_variance).unsqueeze(-1) * variance_noise
        else:
            variance = (self._get_variance(t, predicted_variance=predicted_variance) ** 0.5).unsqueeze(-1) * variance_noise

        pred_prev_sample = torch.where(
            (t > 0).unsqueeze(-1),
    ...
    def add_noise(
        self,
        original_samples: torch.FloatTensor,    #(B, T, D)
        noise: torch.FloatTensor,   # (B, T, D)
        timesteps: torch.IntTensor, # (B, T)
    ) -> torch.FloatTensor:
        ...
        while len(sqrt_one_minus_alpha_prod.shape) < len(original_samples.shape):
            sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.unsqueeze(-1) # (B, T, 1)
...
class DDIMTEDiScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        **kwargs,
        ...
        alpha_prod_t = self.alphas_cumprod[timestep] # (B, T)
        #alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else self.final_alpha_cumprod
        alpha_prod_t_prev = torch.where(prev_timestep >= 0,
            self.alphas_cumprod[prev_timestep],
            self.final_alpha_cumprod
        ...
        beta_prod_t = beta_prod_t.unsqueeze(-1) # (B, T, 1)
        alpha_prod_t = alpha_prod_t.unsqueeze(-1) # (B, T, 1)
        alpha_prod_t_prev = alpha_prod_t_prev.unsqueeze(-1) # (B, T, 1)
        ...
        variance = self._get_variance(timestep, prev_timestep).unsqueeze(-1) #(B, T, 1)
