...
from diffusion_policy.env.pusht_memory.pymunk_override import DrawOptions
...
class PushTMemoryEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(
            low=np.array([0,0,0,0,0,-1], dtype=np.float64),
            high=np.array([ws,ws,ws,ws,np.pi*2,1], dtype=np.float64),
            shape=(6,),
            dtype=np.float64
        )
    ...
    def _get_obs(self):
        ...
        # If goal is visible, include the true index (0 or 1), else use -1 to indicate masking
        if not self.hide_goal:
            obs.append(float(self.chosen_goal_idx))
        else:
            obs.append(-1.0)  # Distinct sentinel value indicating masked goal
    ...
    def _get_info(self):
        ...
        info = {
            ...
            'goal_pose_idx': self.chosen_goal_idx if not self.hide_goal else -1,
            'n_contacts': n_contact_points_per_step,
            'chose_correct': self.chose_correct}
        return info
